...
export default function Certifications({ onSelectCertificate }) {
  ...
  return (
    <section id="certifications" className="py-20 border-b border-zinc-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-left mb-10">
          ...
          <p className="text-sm font-mono text-zinc-500 mt-1">
            Verified academic credentials across Artificial Intelligence, Java OOP, JavaScript, React, Maven, and Google Cloud.
          </p>
        </div>

        {/* Compact, Perfectly Proportioned Certificate Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 text-left">
          {certs.map((cert) => (
            <div
              key={cert.id}
              className="rounded-xl overflow-hidden bg-zinc-950 border border-zinc-900 hover:border-zinc-700 transition-all flex flex-col justify-between group"
            >
              <div>
                {/* Proportioned Preview Frame */}
                <div
                  onClick={() => onSelectCertificate(cert)}
                  className="relative aspect-[16/11] w-full overflow-hidden bg-zinc-900 border-b border-zinc-900 cursor-pointer"
                >
                  <img
                    src={cert.image}
                    alt={cert.name}
                    ...
                    loading="lazy"
                  />
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-1.5 text-white font-mono text-xs">
                ...
                {/* Info */}
                <div className="p-4">
                  <h3 className="text-sm font-bold text-white font-sans line-clamp-2 group-hover:text-zinc-200">
                    {cert.name}
                  </h3>
